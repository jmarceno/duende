module imports_cases.wildcard.use_wildcard;

import a = imports_cases.wildcard.a;
alias foo = a.foo;
alias bar = a.bar;
...
int main() {
    (() { static if (is(typeof(writeln(foo())) == void)) { writeln(foo()); } else { auto __du_tmp = writeln(foo()); duende_eval(__du_tmp); } })();
    (() { static if (is(typeof(writeln(bar())) == void)) { writeln(bar()); } else { auto __du_tmp = writeln(bar()); duende_eval(__du_tmp); } })();
    return 0;
}
